#!/usr/bin/env node
/**
 * Record a ~3:00 silent product demo matching VIDEO-SCRIPT.md beats.
 *
 * Uses the local viewer (mock is fine for the click-through). The mosaic
 * receipt beat loads the committed live demo engagement so Transform
 * inferences are real GPT-5.6 output.
 *
 * Prerequisites:
 *   PRIVILEGE_MOCK=1 .venv/bin/python -m src.server_http --db demo/demo-vault.sqlite3 --mock
 *
 * Usage:
 *   node record-demo-3min.mjs
 *
 * Outputs:
 *   docs/media/privilege-demo-3min.webm
 *   docs/media/privilege-demo-3min.mp4  (if ffmpeg available)
 *
 * Add voiceover later from VIDEO-SCRIPT.md — this file is silent.
 */
...
const TMP = join(MEDIA, '.demo-record-tmp');
...
async function main() {
  ...
  await markLiveBadge(page, 'Demo recording — click-through may use mock; Transform receipts are live GPT-5.6.');
  ...
  await page.evaluate(() => {
    ...
    card.style.cssText = [
      ...
      'display:flex', 'align-items:center', 'justify-content:center',
      'padding:24px',
      'background:color-mix(in oklch, var(--canvas) 40%, black 60%)',
    ].join(';');
    card.innerHTML = `
      <div style="width:min(560px,100%);background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:28px;">
        <div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;color:var(--faint);margin-bottom:10px;">Privilege</div>
  ...
  const webmOut = join(MEDIA, 'privilege-demo-3min.webm');
  const mp4Out = join(MEDIA, 'privilege-demo-3min.mp4');
  copyFileSync(webmSrc, webmOut);
  rmSync(TMP, { recursive: true, force: true });

  console.log(`Wrote ${webmOut}`);
  if (toMp4(webmOut, mp4Out)) console.log(`Wrote ${mp4Out}`);
  console.log(`Target ~180s; recorded ${elapsed.toFixed(1)}s. Silent — narrate from VIDEO-SCRIPT.md.`);
}
